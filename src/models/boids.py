...
class ResearchBoidsModel(BaseModel):
    ...
    def compute_force(self, agent, neighbors, context) -> np.ndarray:
        ...
        local_force = (
            self._cohesion(agent, neighbors, config) * config.cohesion * multipliers["cohesion"]
            + self._alignment(agent, neighbors, config) * config.alignment * multipliers["alignment"]
            + self._separation(agent, neighbors, config) * config.separation * multipliers["separation"]
        )
    ...
    def _cohesion(self, agent, neighbors, config) -> np.ndarray:
        ...
        desired_velocity = normalize(centroid - agent.position, fallback=agent.direction) * float(config.speed)
    ...
    def _alignment(self, agent, neighbors, config) -> np.ndarray:
        ...
        desired_velocity = normalize(mean_velocity, fallback=agent.direction) * float(config.speed)
        return desired_velocity - agent.velocity
    ...
    def _separation(self, agent, neighbors, config) -> np.ndarray:
        ...
        if np.linalg.norm(repel) < 1e-8:
            return np.zeros(2, dtype=float)
        desired_velocity = normalize(repel, fallback=agent.direction) * float(config.speed)
        return desired_velocity - agent.velocity
